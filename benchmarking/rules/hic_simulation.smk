...
"""
# just using head instead of seqtk sanple, which seems to be buggy (random seed not always giving same output)
rule subsample_real_hic_data2:
    input:
        RawRealHicReads.path(individual="real") + "/reads{pair}.fq.gz",
    output:
        HiCReads.path(individual="real") + "/reads{pair}.fq.gz",
    params:
        n_reads = lambda wildcards: int(wildcards.n_reads) // 2,
        n_lines = lambda wildcards: int(wildcards.n_reads) // 2 * 4,
    shell:
        #"seqtk sample -s {wildcards.seed} {input} {params.n_reads} | gzip -c > {output}"
        "zcat {input} | head -n {params.n_lines} | bgzip > {output}"
"""
